...
{
	static double coe = 0.00001749f; //individual calibration required
...
void SpiDispInit()
{
	...
	GPIO_PinModeSet(TOUCH_RES_PORT, TOUCH_RES_PIN, gpioModePushPull, 1); //Touch Reset
...
void UartInit()
{
	...
	GPIO_PinModeSet(MCU_OK_PORT, MCU_OK_PIN, gpioModePushPull, 0); //MCU OK - TURN OFF GREEN
...
void FreqSet(uint8_t f)
{
	...
	else
	{
		...
		case 0:
			...
			break;
		case 1:
			CMU_ClockDivSet(cmuClock_CORE, 8);	
			CMU_ClockSelectSet(cmuClock_SYSCLK, cmuSelect_HFXO);
			break;
		case 2:
			CMU_ClockDivSet(cmuClock_CORE, 4);	
			CMU_ClockSelectSet(cmuClock_SYSCLK, cmuSelect_HFXO);
			break;
		case 3:
			CMU_ClockDivSet(cmuClock_CORE, 2);	
			CMU_ClockSelectSet(cmuClock_SYSCLK, cmuSelect_HFXO);
			break;
		case 4:
			CMU_ClockDivSet(cmuClock_CORE, 1);	
