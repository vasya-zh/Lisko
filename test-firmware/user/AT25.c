...
void SpiFlashDispInit()
{
	...
	GPIO_PinModeSet(DISP_RES_PORT, DISP_RES_PIN, gpioModePushPull, 0); //DISP RES
	GPIO_PinModeSet(SPI_BUSY_PORT, SPI_BUSY_PIN, gpioModeInputPull, 0); //DISP BUSY
	GPIO_PinModeSet(SPI_CS_DISP_PORT, SPI_CS_DISP_PIN, gpioModePushPull, 1); //DISP CS
...
uint32_t SpiFlashGetSize()
{
	...
	for (i = 0; i < 11; i++)
	{
		SizeH =  USART_SpiTransfer(SPI_DISP_FLASH, 0); //header size
		uPrintf("%02X ", SizeH);
	}
	...
	i = USART_SpiTransfer(SPI_DISP_FLASH, 0); //block align
	uPrintf("%02X ", i);
	for (i = 0; i < (SizeH << 3) + 4; i++) //jump to memsize
	{
		j =  USART_SpiTransfer(SPI_DISP_FLASH, 0);
		uPrintf("%02X ", j);
	}
	for (i = 0; i < 4; i++)
	{
		SizeH =  USART_SpiTransfer(SPI_DISP_FLASH, 0);
		uPrintf("%02X ", SizeH);
		r >>= 8;
		r |= SizeH << 24;
	}
