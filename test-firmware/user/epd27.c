...
void Epaper_Read(uint8_t cmd, uint8_t * data, uint8_t len)
{
	...
	USART_BaudrateSyncSet(SPI_PERF, 0, 1000000);
	while (len--)
	{
		*data++ = USART_SpiTransfer(SPI_PERF, 0x00);
	}
	
	DISP_CS(1);
	USART_BaudrateSyncSet(SPI_PERF, 0, 10000000);
}

uint8_t EPD_ReadStatus()
{
	uint8_t i;
	...
	DISP_CS(1);
	DISP_CS(0);
	USART_SpiTransfer(SPI_PERF, 0x2F);
	DISP_DC(1);

	USART_BaudrateSyncSet(SPI_PERF, 0, 1000000);
	i = USART_SpiTransfer(SPI_PERF, 0x00);
	USART_BaudrateSyncSet(SPI_PERF, 0, 10000000);
	DISP_CS(1);
	return i;
}
...
void EPD_PartWriteFullNoWait(bool init)
{
	if (init)
	{
		EPD_reset();	
